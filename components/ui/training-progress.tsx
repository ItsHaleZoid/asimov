...
export default function TrainingProgress({ job, onCancel, onBackToModels, enablePolling = false }: TrainingProgressProps) {
  ...
  useEffect(() => {
    ...
    if (enablePolling) {
      const fetchJobStatus = async () => {
        try {
          const response = await fetch(`/api/training-jobs/${job.id}`);
          if (response.ok) {
            const updatedJob = await response.json();
            // Update job through parent component if needed
          }
        } catch (error) {
